...
/**
 * new ClassListHelper(new ClassListResponse(){
 * @Override
 * public void processFinish(String output) {
 *     DatabaseHelper dbHelper = new DatabaseHelper();
 *     dbHelper.onClassListHelperCompleted(output);
 *     ArrayList<HashMap<String, String>> classesList = dbHelper.getClassList();
 *
 ...
 * @version 1.0
 */
public class ClassListHelper extends AsyncTask<Void, Void, String> {
    String response = "";
    Context context;
    Long totalBytes;
    ProgressDialog pd;
    ...
    /**
     * This class extends AsyncTask and will run a 'GET' request for the
     * list of classes. An example can be found in ClassListHelper.java
     *
     * @param delegate The response delegate (see above example)
     * @param context Context of the activity to be passed.
     */
    public ClassListHelper(AsyncResponder delegate, Context context){
    ...
    /**
     * Gets the JSON file of classes from the backend database.
     * @throws IOException
     */
